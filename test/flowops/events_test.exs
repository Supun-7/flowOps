defmodule Flowops.EventsTest do
  ...
  describe "events" do
    ...
    test "create_event/2 with valid data creates a event" do
      valid_attrs = %{description: "some description", title: "some title", location: "some location", start_time: ~N[2026-05-25 18:05:00], end_time: ~N[2026-05-25 18:05:00]}
      scope = user_scope_fixture()
      ...
      assert event.title == "some title"
      assert event.location == "some location"
      assert event.start_time == ~N[2026-05-25 18:05:00]
      assert event.end_time == ~N[2026-05-25 18:05:00]
      assert event.user_id == scope.user.id
    end
    ...
    test "update_event/3 with valid data updates the event" do
      scope = user_scope_fixture()
      event = event_fixture(scope)
      update_attrs = %{description: "some updated description", title: "some updated title", location: "some updated location", start_time: ~N[2026-05-26 18:05:00], end_time: ~N[2026-05-26 18:05:00]}

      assert {:ok, %Event{} = event} = Events.update_event(scope, event, update_attrs)
      assert event.description == "some updated description"
      assert event.title == "some updated title"
      assert event.location == "some updated location"
      assert event.start_time == ~N[2026-05-26 18:05:00]
      assert event.end_time == ~N[2026-05-26 18:05:00]
    end
